    ...
    T: ToolExecutor,
{
    ...
    pub fn run_turn(
        ...
        loop {
            ...
            // Only assign if we haven't already recorded a preflight compaction event
            // in a previous iteration, so the first one is preserved across loop cycles.
            if preflight_auto_compaction.is_none() {
...
mod tests {
    use super::{
        ...
        PREFLIGHT_COMPACTION_TOKENS_ENV_VAR, PromptCacheEvent, RuntimeError, StaticToolExecutor,
    ...
    fn preflight_compaction_threshold_comes_from_env() {
        // Capture any pre-existing env var value so we can restore it.
        let original = std::env::var(PREFLIGHT_COMPACTION_TOKENS_ENV_VAR).ok();

        // Ensure the env var is unset before checking the default.
        if original.is_some() {
        ...
        // Verify that an explicit value is parsed correctly.
        std::env::set_var(PREFLIGHT_COMPACTION_TOKENS_ENV_VAR, "99999");
        assert_eq!(preflight_compaction_threshold_from_env(), 99_999);

        // Restore original env state.
        match original {
            Some(val) => std::env::set_var(PREFLIGHT_COMPACTION_TOKENS_ENV_VAR, val),
            None => {
                std::env::remove_var(PREFLIGHT_COMPACTION_TOKENS_ENV_VAR);
            }
        }
